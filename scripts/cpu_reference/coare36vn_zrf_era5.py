...
# input us = 0*u here;
    ...
    L10 = zu / zetau
    gf = ut / np.maximum(np.abs(du), 0.1)
    ...
    for i in np.arange(1,nits+1).reshape(-1):
        ...
        ik = np.array(np.where((rr[iice] > 2.5) & (rr[iice] <= 1000)))
    ...
    wc_frac = 0.00073 * np.maximum(U10N - 2, 0) ** 1.43
...
def psit_26(zeta = None): 
    ...
    safe_term = np.maximum(1 + 0.6667 * zeta, 0)
    psi = - (safe_term ** 1.5 + np.multiply(0.6667 * (zeta - 14.28),np.exp(- dzeta)) + 8.525)
